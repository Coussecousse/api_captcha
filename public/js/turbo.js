window.onload = e => {
    ...
    // if (count === 0) customElements.define('puzzle-captcha', PuzzleCaptcha);
...
addEventListener("turbo:submit-end", async () => {
    console.log('yo');
    const imageContainer = document.querySelector('.captcha-image');
    // imageContainer.style.backgroundImage = ``;
    console.log(imageContainer.style.backgrund);
    const imageBackground = imageContainer.style.background;
    const oldUrl = imageBackground.slice(5, -2);
    console.log(oldUrl);

    URL.revokeObjectURL(oldUrl);
    (async () => {
        console.log('bouh');
        const captchaPuzzle = document.querySelector('puzzle-captcha');
        console.log(captchaPuzzle)
        const src = captchaPuzzle.getAttribute('src');
        ...
        try {
            ...
            const imageContainer = document.querySelector('.captcha-image');
            imageContainer.style.background = `url(${url}) center no-repeat`;

            const pieces = document.querySelectorAll('.captcha-piece');
            pieces.forEach((piece, i) => {
                console.log(`url(${url})`);
                piece.style.backgroundImage = `url(${url})`;
            });

            captchaPuzzle.setAttribute('src', url);
            console.log(captchaPuzzle.getAttribute('src'));
